...
(:
 ...
 The source model was procycling-0.0.1
 ...
 Generated at timestamp: 2019-01-18T11:33:41.840714+01:00
 :)
...
) as map:map
{
    ...
    let $id  :=             $source-node/prodId ! xs:string(.)
    let $name  :=             $source-node/name ! xs:string(.)
    let $price  :=             $source-node/price ! xs:double(.)
    let $weightKg  :=             $source-node/weightKg ! xs:double(.)
    let $weightLb  :=             $source-node/weightLb ! xs:double(.)
    let $material  :=             $source-node/madeOf ! xs:string(.)
    let $gender  :=             $source-node/gender ! xs:string(.)
    let $color  :=             $source-node/color ! xs:string(.)
    let $description  :=             $source-node/description ! xs:string(.)
    let $speeds  :=             $source-node/speeds ! xs:decimal(.)
    (: The following property is a local reference.  :)
    let $recommend  :=             es:extract-array($source-node/recommend, procycling:extract-instance-Product#1) 
    ...
    then $instance
        ...
        =>   map:with('id', $id)
        =>   map:with('name', $name)
        =>   map:with('price', $price)
        =>es:optional('weightKg', $weightKg)
        =>es:optional('weightLb', $weightLb)
        =>es:optional('material', $material)
        =>es:optional('gender', $gender)
        =>es:optional('color', $color)
        =>es:optional('description', $description)
        =>es:optional('speeds', $speeds)
        =>es:optional('recommend', $recommend)
};
...
(:~
 ...
 :)
declare function procycling:instance-to-canonical(
...
) as node()
{
        ...
        then xdmp:to-json( procycling:canonicalize($entity-instance) )/node()
        else procycling:instance-to-canonical-xml($entity-instance)
};
...
(:~
 ...
 :)
declare private function procycling:canonicalize(
    $entity-instance as map:map
) as map:map
{
    json:object()
                    ...
                    return
                        ...
                        case json:object
                            return
                                ...
                                else map:put($m, $key, procycling:canonicalize($instance-property))
                        ...
                        case json:array
                            return
                                ...
                                return
                                    ...
                                    then json:set-item-at($instance-property, $i, procycling:canonicalize($val))
                                    else (),
...
(:~
 ...
 : @return An XML element that encodes the instance.
 :)
declare private function procycling:instance-to-canonical-xml(
    $entity-instance as map:map
) as element()
{
                ...
                return
                    ...
                    case json:object+
                        return
                            ...
                            return element { $ns-key } { procycling:instance-to-canonical-xml($prop) }
                    (: An array can also treated as multiple elements :)
                    case json:array
                        return
...
{
    let $canonical := procycling:instance-to-canonical($entity-instance, $envelope-format)
...
{
    procycling:instance-to-envelope($entity-instance, "xml")
};
